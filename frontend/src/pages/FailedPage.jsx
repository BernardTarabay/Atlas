 ...
 * FAILED.
 *
 * THE NAME. This screen was called "Triage", which is what the queue is to
 * somebody who built it and nothing at all to somebody who opens it. "Failed"
 * is what the rows on it have in common, and it is a word anyone reads once
 * and understands. The BACKEND still says triage -- the endpoints, the
 * repository, the reason vocabulary -- deliberately: that domain also contains
 * a genuinely different "email triage" classifier, and renaming its columns
 * and indexes to change a label would be a rewrite, not a rename. The boundary
 * is the UI, and it is crossed only in this file and in the navigation.
 *
...
export function FailedPage() {
   ...
   * as doing nothing.
   *
   * What clears this queue is a decision: put the file somewhere, or get rid
   * of it. Those are what this page offers now. The retry endpoint still exists
  ...
  return (
    <div>
      <PageHeader
        title="Failed"
        description="Everything the pipeline could not process, with the reason — so a failure is a queue you can work through, not a file that quietly disappeared."
        actions={
          <button
      ...
      {loading ? (
        ...
        <ErrorState error={rowsError} onRetry={reload} title="Couldn't load the failed queue" />
      ) : !rows?.length ? (
        <EmptyState
          icon={LifeBuoy}
          title={reason ? "Nothing under this reason" : "Nothing has failed"}
          description={
            reason
